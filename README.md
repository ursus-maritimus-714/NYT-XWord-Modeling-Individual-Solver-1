...
* 'Solver Past Performance Features' (n=6) included a subset (2) of features capturing short-term IS1 past performance. A number of temporal integration windows and time-decay weighting curves were tested in [preliminary univariate linear regression modeling](), and inclusion of the 8 puzzle day-specific puzzles immediately prior to (but *not* including) the given puzzle being predicted yielded the lowest mean training error by root mean square error (RMSE). Later testing with the full feature set revealed, however, that predictions were slightly better when including *only* long-term features from this class. Thus, the ultimate 'Best Model' included only smooth decay-weighted, long-term GMS past performance features ('GMS_RPB_l40' and 'GMS_RPB_l40_stdev'; see Discussion for commentary on this subject).
